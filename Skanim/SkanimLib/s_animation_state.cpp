...
namespace Skanim
{
    ...
    AnimationState::AnimationState(const String &name, const AnimationClip *animation_clip, 
    ...
    void AnimationState::advanceTime(long elapsed_time)
    {
        ...
        // Current local time may exceeds the time range of the animation clip's time.
        // If loop play mode is off, we need to clamp current local time in the animation clip's time range.
        // Otherwise the current local time need to be divide by animation's time length to keep it in valid range 
        // Which causes "Jump" on local time line.
        // Set the jump flag when jump happens so we can deal with this special situation in _updateCurrentPose() later.
        const long animation_clip_time_length = m_animation_clip->getTimeLength();

        if (m_current_local_time < 0 || m_current_local_time > animation_clip_time_length) {
            if (!m_is_looping) {
                m_current_local_time = Math::clamp(m_current_local_time, 0, animation_clip_time_length);
                m_jump_flag = JUMP_FLAG_NONE;
            }
            else {
    ...
    void AnimationState::reset()
    {
        ...
        m_current_local_time = 0;
        // Update the current pose so the current pose is the first pose of the animation.
        _updateCurrentPose();
    ...
    void AnimationState::_updateCurrentPose()
    {
        ...
        // There could be local time jump due to loop playing, if jump happens we should handle
        ...
        if (m_jump_flag == JUMP_FLAG_NONE) {
            ...
            delta_root_transform = Transform::combine(second_delta_root_transform, first_delta_root_transform);
        }
    ...
    {
        // Extract the root transform of the begin pose and the end pose in the animation clip.
        m_begining_root_transform = m_animation_clip->extractPose(0).getJointTransform(0);
        m_end_root_transform = m_animation_clip->extractPose(m_animation_clip->getTimeLength()).getJointTransform(0);
    }
